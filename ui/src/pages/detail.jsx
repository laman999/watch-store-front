...
function Detail() {
  ...
  const addToCart = () => {
    ...
    if (!userId) {
      alert("Səbətə əlavə etmək üçün öncə giriş edin!");
      return;
    }

    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    const existing = cart.find(item => item.id === watch.id && item.userId === userId);

    if (existing) {
      existing.quantity += 1;
    } else {
      cart.push({ ...watch, userId: userId, quantity: 1 });
    }
  ...
  return (
    <div className="min-h-screen bg-black dark:bg-white text-white pt-40 px-10 transition-colors duration-500">
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-20 mx-auto max-w-6xl'>
        ...
        <div className='flex flex-col justify-center'>
          ...
          <div className='border-t border-zinc-800 pt-8 space-y-6'>
            <p className='text-zinc-400 italic leading-relaxed'> 
              {watch.category === 'men' ? t('desc_men') : t('desc_women')}
            </p>
