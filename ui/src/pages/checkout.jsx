...
const Checkout = () => {
  ...
  const calculatedTotal = cartItems.reduce((acc, item) => acc + (item.price * (item.quantity || 1)), 0);
  ...
  const handleOrder = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(`Sifarişiniz qəbul edildi! Sifariş nömrəniz: #${generatedId}`);
        localStorage.removeItem("cart");
        window.dispatchEvent(new Event("cartUpdated")); 
        
        navigate("/track-order", { state: { id: generatedId, success: true } }); 
      }
    } catch (err) {
      ...
      <div className="min-h-screen bg-[#0a0a0c] text-white flex items-center justify-center p-4 relative overflow-hidden font-sans">
        ...
        <div className="relative z-10 w-full max-w-md bg-white/5 backdrop-blur-xl p-8 rounded-[30px] border border-white/10 shadow-2xl">
          ...
          <div className="mb-6 p-4 bg-white/5 rounded-2xl border border-white/5 flex justify-between items-center">
             <span className="text-[10px] uppercase tracking-widest text-gray-400">Ümumi Məbləğ:</span>
             <span className="text-[#D4AF37] font-bold text-lg">${calculatedTotal.toLocaleString()}</span>
          </div>
          ...
          <form onSubmit={handleOrder} className="space-y-5">
            ...
            <div className="relative">
              ...
              <div className="absolute right-4 top-4 flex gap-1">
                <div className="w-6 h-4 bg-red-500/80 rounded-sm shadow-sm"></div>
                <div className="w-6 h-4 bg-yellow-500/80 rounded-sm shadow-sm"></div>
              </div>
            </div>
            ...
            <button type="submit" className="w-full mt-4 bg-gradient-to-r from-purple-600 to-blue-500 hover:from-purple-500 hover:to-blue-400 text-white font-medium py-4 rounded-2xl shadow-lg shadow-purple-500/20 transition-all active:scale-95 uppercase text-xs tracking-widest cursor-pointer">
              Sifarişi Tamamla
            </button>
          </form>
        </div>
